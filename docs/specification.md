...
| AC-4.4 | `command-run.sh` moves a draft dropped into `drafts/` to `<slug>/draft.md`, tracked or not, commits it and leaves a clean tree; a hand-written `<slug>/spec.md` arms at `REVIEW-SPEC` | selftest |
...
| AC-5.3 | `agents/task.md` exists, is named `task`, and never calls a `slug_*` helper, `slug_set_phase.sh` or `log.sh` | selftest |
