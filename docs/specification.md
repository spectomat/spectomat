...
| `references/gates.md` | how the task agent reads the gates log and debugs a red gate, cited by `agents/task.md` |
...
| D33 | `state.json` carries `current: {phase, slug}`, the last working verdict handed out, written by the Stop hook in the write that bumps `iteration` and by `arm_flow` for the first iteration; a `RECOVER` or `FINISH` verdict never replaces it, and the picker hands `current.slug` to a dirty-tree `RECOVER` | the picker writing it itself; a `RECOVER` with no slug, the janitor inferring the slug and phase from the dirt | a dirty tree is an iteration that died mid-phase, and which slug and phase died was known the moment it was dispatched but kept nowhere: the janitor had to guess both from the changes, and fell back to logging against `floor` when it could not. The Stop hook already runs the picker and already writes `state.json` once per iteration, so recording the verdict costs no extra process or write, and the picker stays pure — it only reads the field, and `/spectomat:status` can still run it on demand. A verdict is still re-run, never remembered, for choosing the next phase: `current` is read for `RECOVER`'s slug and for `/spectomat:status`, and nothing else. The cost is one more field that must skip `RECOVER`, or a janitor that died mid-work would overwrite the slug it was cleaning up after |
| D34 | The Stop hook runs the picker once via `ask_picker` (to decide `FINISH` vs continue) and `pointer_prompt()` embeds that same block in the text fed back, so the dispatched session reads the verdict rather than running `phase.sh` again | the session running `phase.sh` itself as step 1 of the iteration, per §3.2's original wording | the hook already runs the picker every iteration before it can tell `FINISH` from a working verdict; a second run by the session was pure duplication of a call the idempotency guarantee (§3.1) already promises cannot answer differently, since nothing mutates `state.json` or the tree between the two. Removing it drops one `phase.sh` subprocess and one blind tool call per iteration at no cost: the block the session receives is byte-identical to what a second run would have produced. `ask_picker` (`utils.sh`) now also keeps the raw block in `PICK_BLOCK` alongside the parsed `PICK_PHASE`/`PICK_SLUG` it always set, so `command-run.sh`'s arm-time call is unaffected |
