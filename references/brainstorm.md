...
**A draft that asks for several independent systems** is neither brainstormed into one spec nor silently cut to one: it is a strike. Write no spec and leave the draft in place — the operator splits it — then follow [Three strikes](./three-strikes.md) for `SPECIFY`, with the reason `draft asks for K independent systems: a, b`.
