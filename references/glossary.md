...
- **Task agent** is `spectomat:task` (`agents/task.md`): a fresh subagent per task, dispatched by the `IMPLEMENT` phase agent with three lines — slug, task file path (`.spectomat/<slug>/tasks/task-NN-<name>.md`), gates log path. It reads only its task file, the snippets and the code, never the plan, the spec, the contract or `memory.md`; builds under TDD, runs the gates into `.spectomat/work/<slug>/task-NN.gates.log`, makes the task's one `feat` commit, and reports `DONE` or `FAILED`. It touches no floor state: `state.json`, `log.md`, `tasks.json`, `ruling.md` and `memory.md` are the phase agent's to write.
