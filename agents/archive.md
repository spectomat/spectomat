...
1. Run `bash <plugin_root>/scripts/agent-archive.sh <slug>`.
...
- All of that lives in `scripts/agent-archive.sh`, and it stays there — a script that exits non-zero on a failing gate is stronger evidence than an agent claiming the gate passed.
...
- Do not move a file, run a gate, or write to `state.json` yourself. `agent-archive.sh` already does all three; duplicating any of it is the one thing this brief exists to prevent.
- Do not touch a file outside what `agent-archive.sh` itself touched.
