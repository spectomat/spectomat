...
- Every task file carries five numbered steps; the `IMPLEMENT` phase finds its work from the ledger `.spectomat/<slug>/tasks.json`, not from the task files' own text and not from `state.json`.
...
| `dependsOn` | the ids of every task whose Produced Interfaces this task consumes; only lower ids, `[]` for the first |
