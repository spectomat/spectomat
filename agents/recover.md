...
Read then `./.spectomat/memory.md`.

Find which of the three cases you are in, and do only that one:

**A dirty tree.** `git status --porcelain` is not silent, so a previous iteration died mid-phase. Inspect the changes. If they are a phase all but finished, finish it and commit it under that phase's own message. When you commit the crashed phase's work, also apply that phase's `state.json` transition using whichever of `<plugin_root>/scripts/slug_set_phase.sh`, `slug_start_tasks`, `slug_task_done`, or `slug_add_tasks` (the latter three from `scripts/utils.sh`) matches the phase that crashed — the same helper that phase's own brief would have called. If they are partial or you cannot tell what they were for, discard them — `git checkout -- .` and `git clean -fd` the paths under `.spectomat/` and the paths the task files name. Never discard a change outside those paths; report it instead and stop.
