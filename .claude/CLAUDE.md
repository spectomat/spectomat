...
- `state.md` is gitignored and re-rendered each run. Its frontmatter (`loop`, `max_loops`, `session_id`) drives the Stop hook; its body is the pointer prompt, which carries `PLUGIN_ROOT` (absolute plugin path, locating `agents/looper.md` and `references/`). `GATES` (one `&&` chain compiled by `gates.sh` from `package.json` scripts, or an echo when none) is rendered into the contract.

`hooks/hooks.json` wires `scripts/stop-hook.sh`: while the state file exists for the session that started it (session id match, so other sessions in the same project are untouched), it blocks exit, bumps `loop:` and feeds the pointer back. The pointer makes the session launch one fresh looper per loop: `subagent_type` `spectomat:looper` when the plugin's agents are listed, else `general-purpose` with the body of `agents/looper.md` as the brief; either way the task line names the `references/` path. The session itself does no factory work. `agents/looper.md` holds the brief and nothing else: no plugin path (the task line carries it), no `{{KEY}}` (it is not rendered). The hook releases on the exact promise, the cap or a corrupt file. Derived from Anthropic's ralph-loop; the state file name differs so both plugins coexist.
...
- A change to the state file keys or the pointer text must keep `stop-hook.sh`, `print.sh`, `cancel.sh` and `templates/state.md` in step; a change to what a loop does belongs in `agents/looper.md` and the contract, not in the pointer.
