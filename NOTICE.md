Spectomat is MIT licensed (`LICENSE`). The parts below come from other projects
...
`docs/.spectomat/state.md`, the pointer prompt is fixed, and the setup
prepares the factory floor before arming the hook.

The skills `writing-plans`, `test-driven-development` and
`systematic-debugging`, and the `executing-tasks` skill (a merge of
